...
@section('title', __('script.Create Script') )
@section('content')
    <link rel="stylesheet" href="{{ asset('css/script/main.css') }}">
    <form method="POST" action="{{route('script.postCreate')}}" enctype="multipart/form-data">
        @csrf
        <div class="form-group row">
            <label for="name" class="col-md-4 col-form-label text-md-right">{{ __('script.Name') }}</label>

            <div class="col-md-6">
                <input id="username" type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" autocomplete="name" autofocus>
        ...
        <div class="form-group row">
            ...
            <div class="col-md-6">
                <textarea style="height: 100px;" id="description" type="text" class="form-control @error('description') is-invalid @enderror" name="description" autocomplete="description" autofocus>{{ old('description') }}</textarea>
        ...
        <div class="form-group row">
            ...
            <div class="col-md-6" >
                <div class="input-group">
                    <input id="fileName" name="file_name" type="text" class="form-control  @error('fileName') is-invalid @enderror" readonly value="{{ old('file_name') }}" autocomplete="fileName" autofocus/>
                    <input class="d-none" id="inputFile" type="file"  name="fileName" value="" accept="py">
        ...
        <div class="form-group row">
            ...
            <div class="col-md-6">
                <select id="isActive" class="form-control select2bs4 @error('isActive') is-invalid @enderror" style="width: 100%;" name="isActive">
                    <option value="1" {{ old('isActive') == '1' ? 'selected' : '' }}>{{__('script.execFile')}}</option>
                    <option value="0" {{ old('isActive') == '0' ? 'selected' : '' }}>{{__('script.supportFile')}}</option>
                </select>
